...
parsegraph_AudioKeyboard.prototype.prepare = function(gl, glyphAtlas, shaders)
{
    this._gl = gl;
...
parsegraph_AudioKeyboard.prototype.paint = function()
{
    ...
    var highlight = 0.2;
    var whiteBorder = new parsegraph_Color(1-highlight, 1-highlight, 1-highlight, 1);
    var black = new parsegraph_Color(0, 0, 0, 1);
    var blackBorder = new parsegraph_Color(2*highlight, 2*highlight, 2*highlight, 1);
    if(!this._whiteKeyPainter) {
        this._whiteKeyPainter = new parsegraph_BlockPainter(this._gl, this._shaders);
    ...
    if(!this._blackKeyPainter) {
        this._blackKeyPainter = new parsegraph_BlockPainter(this._gl, this._shaders);
    ...
    for(var i = 0; i < maxKeys; ++i) {
        ...
        var wx = this._worldX + whiteKeyWidth * (i+.5);
        var kx = 0;
        switch(i % 7) {
        case 0:
