function showFunction(COUNT) {
  ...
  const DOMAIN_START = -2 * 3.14159;

  // DOMAIN_END is the maximum domain value used.
  const DOMAIN_END = 2 * 3.14159;

  let pos = DOMAIN_START;
  const increment = (DOMAIN_END - DOMAIN_START) / HORIZONTAL_STEPS;
  for (let i = 0; i < HORIZONTAL_STEPS; ++i) {
    const result = Math.sin(pos);
    ...
    if (result < RANGE_START) {
      ...
      const interpolated = (result - RANGE_START) / (RANGE_END - RANGE_START);
      colSize = Math.floor(interpolated * VERTICAL_SIZE);
    }

    caret.push();
    for (let j = 0; j < Math.abs(colSize); ++j) {
      if (colSize > 0) {
