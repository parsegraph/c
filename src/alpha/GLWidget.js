...
export function AlphaGLWidget(belt, window) {
  ...
  let rot = 0;
  for (let i = 0; i < 24; ++i) {
    let q = alphaQuaternionFromAxisAndAngle(1, 0, 0, (rot * Math.PI) / 180);
    rot += 15;
    let p = q.RotatedVector(0, 0, -radius);
    this.sphereCluster.AddBlock(stone, p, 0);
  }

  rot = 0;
  for (let i = 0; i < 24; ++i) {
    let q = alphaQuaternionFromAxisAndAngle(0, 1, 0, (rot * Math.PI) / 180);
    rot += 15;
  ...
  for (let i = 0; i < 10; ++i) {
    ...
    let q = new alpha_Quaternion(x, y, z, w);
    q.Normalize();
