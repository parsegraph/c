...
parsegraph_BurgerMenu.prototype.getIcon = function(x, y)
{
    ...
    if(!this._menuOpened) {
        ...
        if(x < center - parsegraph_MENU_ICON_SIZE/2 || x > center + parsegraph_MENU_ICON_SIZE/2) {
            return null;
        }
    ...
    for(var iconIndex in this._iconLocations) {
        ...
        if(x < iconLocation-parsegraph_MENU_ICON_SIZE/2 || x > iconLocation+parsegraph_MENU_ICON_SIZE/2) {
            continue;
        }
        return iconIndex;
    }
...
parsegraph_BurgerMenu.prototype.drawIcon = function(iconIndex, x, y)
{
    ...
    this._iconPainter.drawTexture(
        iconIndex*parsegraph_MENU_ICON_TEXTURE_SIZE, 0, // iconX, iconY
        parsegraph_MENU_ICON_TEXTURE_SIZE, parsegraph_MENU_ICON_TEXTURE_SIZE, // iconWidth, iconHeight
        x, y,
        parsegraph_MENU_ICON_SIZE, -parsegraph_MENU_ICON_SIZE, // width, height
        1
    );
}
...
parsegraph_BurgerMenu.prototype.paint = function()
{
    ...
    if(!this._iconTexture) {
        ...
        gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, this._iconImage);
        gl.generateMipmap(gl.TEXTURE_2D);
        gl.bindTexture(gl.TEXTURE_2D, null);
    }
    if(!this._iconPainter) {
        this._iconPainter = new parsegraph_TexturePainter(this.window(), this._iconTexture, parsegraph_MENU_ICON_TEXTURE_SIZE*8, parsegraph_MENU_ICON_TEXTURE_SIZE);
    }
