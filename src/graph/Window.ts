...
export default class Window {
	...
	_overlayCtx:WebGLRenderingContext;
	...
	_monitoredTouches:Touch[];
	_touchstartTime:Date;

	_needsUpdate:boolean;

  constructor() {
    this._id = ++windowCount;
     ...
     */
    addEventMethod(this.canvas(), 'DOMMouseScroll', this.onWheel, this, false);
    addEventMethod(this.canvas(), 'mousewheel', this.onWheel, this, false);
  ...
  getTouchByIdentifier(identifier) {
  ...
  removeTouchByIdentifier(identifier) {
    let touch = null;
    for (let i = 0; i < this._monitoredTouches.length; ++i) {
      if (this._monitoredTouches[i].identifier == identifier) {
