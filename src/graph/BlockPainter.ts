...
export default class BlockPainter
{
    ...
    constructor(window:Window) {
        ...
        this._backgroundColor = new Color(1, 1, 1, .15);
    ...
    drawBlock(cx:number, cy:number, width:number, height:number, borderRoundedness:number, borderThickness:number, borderScale:number):void {
        const gl = this._window.gl();
        if(gl.isContextLost()) {
            return;
        }
