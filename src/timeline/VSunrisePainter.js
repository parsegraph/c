...
function parsegraph_VSunrisePainter(gl) {
  ...
  this.u_color = this._gl.getUniformLocation(this._blockProgram, 'u_color');
...
parsegraph_VSunrisePainter.prototype.paint = function(daysRendered) {
  ...
  d.setHours(0, 0, 0, 0);
  let offsetMins = d.getTime() / 1000 / 60;
  d.setHours(12, 0, 0, 0);
  for (let i = 0; i < daysRendered; ++i) {
    const sunTimes = getSunriseAndSunset(
        d,
    ...
    const dayLength = sunTimes[1] - sunTimes[0];
    ...
    this._blockBuffer.appendData(
        ...
        parsegraph_generateRectangleVertices(
            0,
            offsetMins + sunTimes[0] + dayLength / 2,
            2,
            dayLength,
        ),
    );
