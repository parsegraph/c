...
function parsegraph_Node(newType, fromNode, parentDirection) {
  ...
  if (fromNode != null) {
    ...
    if (
      !parsegraph_isNodeDirection(parentDirection) ||
...
parsegraph_Node.prototype.x = function () {
  ...
  return this.nodeParent()._neighbors[
...
parsegraph_Node.prototype.hasNodes = function (axis) {
  ...
  var result = [parsegraph_NULL_NODE_DIRECTION, parsegraph_NULL_NODE_DIRECTION];
...
parsegraph_Node.prototype.hasAnyNodes = function () {
  return (
    this.hasChildAt(parsegraph_DOWNWARD) ||
 ...
 * filterFunc is true.
 */
parsegraph_Node.prototype.traverse = function (
  filterFunc,
  actionFunc,
  thisArg,
  timeout
...
parsegraph_Node.prototype.spawnNode = function (spawnDirection, newType) {
  ...
  if (spawnDirection == this.parentDirection()) {
    throw new Error(
      "Cannot spawn in a node in the parent's direction (" +
        parsegraph_nameNodeDirection(spawnDirection)
    );
  }
  if (this.hasNode(spawnDirection)) {
    throw new Error(
      "Cannot spawn in a node in the already occupied " +
        parsegraph_nameNodeDirection(spawnDirection) +
        " direction. Parent is in the " +
        parsegraph_nameNodeDirection(this.parentDirection()) +
        " direction."
    );
  }
...
parsegraph_Node.prototype.extentSize = function (outPos) {
  ...
  outPos.setHeight(this.extentsAt(parsegraph_FORWARD).boundingValues()[0]);

  // The vertical extents have length in the vertical direction.
  outPos.setWidth(this.extentsAt(parsegraph_DOWNWARD).boundingValues()[0]);
...
parsegraph_Node.prototype.setNodeAlignmentMode = function (
  inDirection,
  newAlignmentMode
) {
...
parsegraph_Node.prototype.verticalSeparation = function (direction) {
  if (
    this.type() == parsegraph_BUD &&
    this.typeAt(direction) == parsegraph_BUD
  ) {
    return (
...
parsegraph_Node.prototype.horizontalSeparation = function (direction) {
  ...
  if (
    ...
    this.nodeAt(direction).type() == parsegraph_BUD &&
  ...
  var inNodeExtents = function (node) {
    if (
      ...
      userScale * node.absoluteX() -
        userScale *
          node.absoluteScale() *
          node.extentOffsetAt(parsegraph_DOWNWARD)
    ) {
      return false;
    }
    if (
    ...
    if (
      ...
      userScale * node.absoluteY() -
        userScale *
          node.absoluteScale() *
          node.extentOffsetAt(parsegraph_FORWARD)
    ) {
      return false;
    }
    if (
...
parsegraph_Node.prototype.sizeWithoutPadding = function (bodySize) {
  ...
  if (this._label && !this._label.isEmpty()) {
    ...
    bodySize[0] =
      this._label.width() *
      (style.fontSize / this._label.glyphAtlas().fontSize());
    bodySize[1] =
      this._label.height() *
      (style.fontSize / this._label.glyphAtlas().fontSize());
  ...
  if (this.hasNode(parsegraph_INWARD)) {
    ...
    if (
      ...
      if (this.label()) {
        ...
        bodySize.setHeight(
          nestedSize.height() * nestedNode.scale() + 2 * this.verticalPadding()
        );
      }
    } else {
      // Align horizontal.
      if (this.label()) {
        ...
        bodySize.setWidth(nestedNode.scale() * nestedSize.width());
      }

      bodySize.setHeight(
        Math.max(
          bodySize.height(),
          nestedNode.scale() * nestedSize.height() + 2 * this.verticalPadding()
        )
      );
    }
  }
...
parsegraph_Node.prototype.commitLayout = function (bodySize) {
  ...
  var initExtent = function (inDirection, length, size, offset) {
    ...
    switch (alignmentMode) {
      ...
      case parsegraph_ALIGN_CENTER: {
        ...
        var positiveLength = parsegraph_findConsecutiveLength(
          child,
          parsegraph_getPositiveNodeDirection(axis)
        );
   ...
   */
  var combineExtents = function (childDirection, alignment, separation) {
    var child = this.nodeAt(childDirection);
     ...
     */
    var combineExtent = function (direction, lengthAdjustment, sizeAdjustment) {
      /*console.log(
                "combineExtent(" +
      ...
      if (this.nodeFit() == parsegraph_NODE_FIT_LOOSE) {
        ...
        if (lengthOffset === 0) {
          ...
          e.setBoundLengthAt(
            0,
            Math.max(e.boundLengthAt(0), bv[0] * scale + lengthOffset)
          );
          e.setBoundSizeAt(
            0,
            Math.max(e.boundSizeAt(0), bv[2] * scale + sizeAdjustment)
          );
        } else if (lengthOffset < 0) {
          //console.log("lengthOffset < 0");
          e.setBoundLengthAt(
            0,
            Math.max(e.boundLengthAt(0) + Math.abs(lengthOffset), bv[0] * scale)
          );
          e.setBoundSizeAt(
            0,
            Math.max(e.boundSizeAt(0), bv[2] * scale + sizeAdjustment)
          ...
          e.setBoundLengthAt(
            0,
            Math.max(e.boundLengthAt(0), lengthOffset + bv[0] * scale)
          );
          e.setBoundSizeAt(
            0,
            Math.max(e.boundSizeAt(0), bv[2] * scale + sizeAdjustment)
   ...
   */
  var layoutSingle = function (direction, allowAxisOverlap) {
    if (!this.hasNode(direction)) {
      return;
    }
  ...
  var layoutAxis = function (
    ...
    var separationBetweenChildren = firstNode
    ...
    var separationFromFirst = this._neighbors[firstDirection].extent.separation(
    ...
    var separationFromSecond = this._neighbors[
      secondDirection
    ].extent.separation(
      secondNode.extentsAt(firstDirection),
    ...
    if (
      ...
      separationFromFirst +=
        this.verticalSeparation(firstDirection) * this.scaleAt(firstDirection);
  ...
  if (
    ...
    if (
      this.canonicalLayoutPreference() == parsegraph_PREFER_PERPENDICULAR_AXIS
    ) {
      // firstDirection and secondDirection, if not NULL_NODE_DIRECTION,
      // indicate a neighboring node in at least that direction.
      var firstDirection = parsegraph_NULL_NODE_DIRECTION;
      var secondDirection = parsegraph_NULL_NODE_DIRECTION;

      // firstAxis indicates the first-axis.
      var firstAxis = parsegraph_getPerpendicularAxis(this.parentDirection());

      // Check for nodes perpendicular to parent's direction
      var hasFirstAxisNodes = this.hasNodes(firstAxis);
      var oppositeFromParent = parsegraph_reverseNodeDirection(
        this.parentDirection()
      );
      layoutAxis.call(this, hasFirstAxisNodes[0], hasFirstAxisNodes[1], false);
  ...
  if (this.hasNode(parsegraph_INWARD)) {
    ...
    if (
      ...
      nestedNode.scale() * nestedSize.width() <
        bodySize.width() -
          2 * (this.horizontalPadding() + this.borderThickness())
    ) {
      this.setPosAt(
        parsegraph_INWARD,
        nestedNode.scale() *
      ...
      this.nodeAlignmentMode(parsegraph_INWARD) ==
        parsegraph_ALIGN_HORIZONTAL &&
      nestedNode.scale() * nestedSize.height() <
        bodySize.height() -
          2 * (this.verticalPadding() + this.borderThickness())
    ) {
      this.setPosAt(
        parsegraph_INWARD,
        bodySize.width() / 2 -
...
parsegraph_Node.prototype.layoutWasChanged = function (changeDirection) {
  ...
  while (node !== null) {
    ...
    if (node.isRoot()) {
      ...
      node = node.nodeParent();
      changeDirection = parsegraph_reverseNodeDirection(node.parentDirection());
    } else {
      // Completed.
      break;
    }
